...
import {
  ...
  type Node,
  type Edge,
} from "../packages/core/dist/schemas.js";
  ...
  return {
    ...
    toThrow: (expectedMessage?: string) => {
      ...
      if (expectedMessage && !actualError.message.includes(expectedMessage)) {
        throw new Error(
          `Expected error message to contain "${expectedMessage}", got "${actualError.message}"`
        );
      }
    },
...
async function runValidationTests() {
  ...
  test("Node with invalid status fails", () => {
    const invalidNode = { ...validNode, status: "invalid-status" as any };
  ...
  test("Node with missing provenance fails", () => {
    const invalidNode = { ...validNode };
    delete (invalidNode as any).provenance;
    expect(() => validateNode(invalidNode)).toThrow();
  });
  ...
  test("Edge with same source and destination fails", () => {
    ...
    expect(() => validateEdge(invalidEdge)).toThrow(
      "Edge source and destination cannot be the same"
    );
  });

  test("Edge with invalid type fails", () => {
    const invalidEdge = { ...validEdge, type: "invalid-type" as any };
  ...
  test("Journal entry with empty event fails", () => {
    ...
    expect(() => validateJournalEntry(invalidEntry)).toThrow(

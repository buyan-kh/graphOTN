...
async function testFinalJournal() {
  ...
  try {
    ...
    const corruptRecoveryResponse = JSON.parse(
      corruptRecoveryResult.content[0].type === "text"
        ? corruptRecoveryResult.content[0].text
        : "{}"
    );
    ...
    if (corruptRecoveryResponse.ok) {
      ...
      throw new Error(
        `Corruption recovery failed: ${corruptRecoveryResponse.error}`
      );
    }
    ...
    console.log(
      "✅ 1. Deleting graph.json then running index rebuilds from journal"
    );
    console.log(
      "✅ 2. Corrupt graph triggers recovery and produces a valid graph"
    );
    console.log(
      "✅ 3. Journal grows on every store action (workspace init logged)"
    );

...
async function testValidationErrors() {
    ...
    try {
        ...
        // Write invalid JSON
        await fs.writeFile(graphPath, '{"nodes": [{"id": "", "invalid": true}], "edges": [], "version": "invalid"}');
        try {
            ...
            const response = JSON.parse(result.content[0].type === "text" ? result.content[0].text : "{}");
            if (!response.ok && response.error && response.error.includes("Invalid graph.json format")) {
        ...
        await fs.writeFile(metaPath, '{"version": "", "created": "invalid-date", "workspace_path": ""}');
        try {
            const result = await client.callTool({
                name: "gotn_index_workspace",
                arguments: { workspace_path: testWorkspace },
...
testValidationErrors().then((success) => {

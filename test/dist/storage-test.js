...
async function testStorage() {
    ...
    try {
        await fs.rm(path.join(testWorkspace, '.gotn'), { recursive: true, force: true });
    ...
    try {
        ...
        console.log("\n📄 Test 3: Check file contents");
        // Check meta.json
        const metaContent = await fs.readFile(path.join(gotnPath, 'meta.json'), 'utf8');
        ...
        // Check graph.json
        const graphContent = await fs.readFile(path.join(gotnPath, 'graph.json'), 'utf8');
        const graph = JSON.parse(graphContent);
        console.log("Graph.json:", graph);
        if (Array.isArray(graph.nodes) && Array.isArray(graph.edges) && graph.version === 1) {
        ...
        const secondInitResponse = JSON.parse(secondInitResult.content[0].type === "text" ? secondInitResult.content[0].text : "{}");
        console.log("Second init response:", secondInitResponse);
        if (secondInitResponse.ok && secondInitResponse.message.includes("already initialized")) {
        ...
        console.log("\n📝 Test 5: Check journal entries");
        const journalContent = await fs.readFile(path.join(gotnPath, 'journal.ndjson'), 'utf8');
        ...
        if (journalLines.length > 0) {
            ...
            if (firstEntry.event === "workspace_initialized" && firstEntry.timestamp && firstEntry.id) {

...
async function testValidationIntegration() {
    ...
    try {
        ...
        try {
            ...
            const invalidInitResponse = JSON.parse(invalidInitResult.content[0].type === "text" ? invalidInitResult.content[0].text : "{}");
        ...
        if (Array.isArray(graph.nodes) && Array.isArray(graph.edges) &&
            typeof graph.version === 'number' && typeof graph.updated === 'string') {
            console.log("✅ Generated graph.json has valid structure");
            console.log(`   Nodes: ${graph.nodes.length}, Edges: ${graph.edges.length}, Version: ${graph.version}`);
        }
        else {
        ...
        if (meta.version && meta.created && meta.updated && meta.workspace_path === testWorkspace) {
        ...
        if (journalLines.length > 0) {
            ...
            if (firstEntry.timestamp && firstEntry.event && firstEntry.id && firstEntry.data) {
...
testValidationIntegration().then((success) => {

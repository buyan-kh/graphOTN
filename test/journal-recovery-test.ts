...
async function testJournalAndRecovery() {
  ...
  try {
    ...
    const storeResponse = JSON.parse(
      storeResult.content[0].type === "text"
    ...
    if (journalLines.length >= 2) {
      // Should have workspace_initialized + add_node
      ...
      throw new Error(
        `Expected at least 2 journal entries, got ${journalLines.length}`
      );
    }
    ...
    const recoveryResponse = JSON.parse(
      recoveryResult.content[0].type === "text"
        ? recoveryResult.content[0].text
        : "{}"
    );
    ...
    if (recoveryResponse.ok) {
      ...
      if (recoveryResponse.nodes_count === 1) {
        ...
        throw new Error(
          `Expected 1 recovered node, got ${recoveryResponse.nodes_count}`
        );
      }
    } else {
    ...
    if (
      recoveredGraph.nodes.length === 1 &&
      recoveredGraph.nodes[0].id === "test-node-1"
    ) {
    ...
    const corruptionRecoveryResponse = JSON.parse(
      corruptionRecoveryResult.content[0].type === "text"
        ? corruptionRecoveryResult.content[0].text
        : "{}"
    );
    ...
    if (corruptionRecoveryResponse.ok) {
      ...
      if (corruptionRecoveryResponse.nodes_count === 1) {
        ...
        throw new Error(
          `Expected 1 recovered node, got ${corruptionRecoveryResponse.nodes_count}`
        );
      }
    } else {
      throw new Error(
        `Recovery from corruption failed: ${corruptionRecoveryResponse.error}`
      );
    }

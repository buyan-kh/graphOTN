...
async function testConcurrentWrites() {
  ...
  if (finalGraph.nodes.length === writeCount) {
    ...
    throw new Error(
      `Expected ${writeCount} nodes, got ${finalGraph.nodes.length}`
    );
  }
  ...
  if (journalLines.length === expectedJournalEntries) {
    ...
    throw new Error(
      `Expected ${expectedJournalEntries} journal entries, got ${journalLines.length}`
    );
  }
  ...
  if (testEventCount === journalWriteCount) {
    ...
    throw new Error(
      `Expected ${journalWriteCount} test events, found ${testEventCount}`
    );
  }

...
import { getCloudStore, getCloudBreakdownEngine, } from "@gotn/core";
...
const GOTN_CLOUD_TOOLS = [
    {
        ...
        inputSchema: {
            ...
            properties: {
                project_id: {
                    type: "string",
                    description: "Unique identifier for the project (e.g., 'my-react-app', 'data-pipeline')",
                },
                project_name: {
                    type: "string",
    ...
    },
    {
        ...
        description: "Break down a large prompt into atomic micro-prompts stored in the cloud",
        inputSchema: {
            type: "object",
            properties: {
                project_id: {
    ...
    },
    {
        ...
        description: "Search nodes by text using semantic similarity within a project",
        inputSchema: {
            type: "object",
            properties: {
                project_id: {
    ...
    try {
        switch (name) {
            ...
            case "gotn_breakdown_prompt":
                return await handleBreakdownPrompt(args);
            case "gotn_get_graph":
                return await handleGetGraph(args);
        ...
        log(`Tool ${name} failed: ${error instanceof Error ? error.message : String(error)}`);
        throw error;
    }
});
// Tool handlers
...
async function handleBreakdownPrompt(args) {
    ...
    try {
        ...
        log(`Stored ${result.nodes.length} nodes and ${result.edges.length} edges in cloud`);
        return {
            content: [
                {
                    type: "text",
        ...
        log(`Breakdown failed: ${error instanceof Error ? error.message : String(error)}`);
        return {
            content: [
                {
                    type: "text",
...
async function handleGetGraph(args) {
...
async function handleSearchNodes(args) {
    ...
    const cloudStore = getCloudStore();
    const nodes = await cloudStore.searchNodes(args.query, args.project_id, args.limit || 10);
    return {
        content: [
            {
                type: "text",

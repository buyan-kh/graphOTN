...
export async function ensureCollection(client) {
    ...
    console.log(`Ensuring collection: ${collectionName} with dimension: ${embeddingDim}`);
    ...
    if (!hasCollection.value) {
        ...
        console.log(`Index created for ${collectionName}.embedding with cosine metric`);
    }
    else {
 ...
 */
export async function upsertVector(client, id, embedding, projectId) {
    const collectionName = "gotn_nodes";
 ...
 */
export async function searchKnn(client, queryVector, topK, projectId) {
    const collectionName = "gotn_nodes";
    console.log(`Searching for ${topK} similar vectors (project: ${projectId || "all"})`);
 ...
 */
export async function getCollectionStats(client) {
    const collectionName = "gotn_nodes";
    ...
    if (stats.stats && Array.isArray(stats.stats)) {
        ...
        const indexedRowCountStat = stats.stats.find((stat) => stat.key === "indexed_row_count");

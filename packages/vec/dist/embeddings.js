...
export class OpenAIEmbedder {
    ...
    constructor(dim = Number(process.env.GOTN_EMBED_DIM) || 1536) {
        this.dim = dim;
     ...
     */
    async embed(text) {
        const res = await this.withRetry(() => this.client.embeddings.create({
            model: this.model,
            input: text,
            dimensions: this.dim,
        }));
        ...
        if (!Array.isArray(v) || v.length !== this.dim) {
            throw new Error(`Unexpected embedding length ${v.length} expected ${this.dim}`);
        }
        return v;
    }
}

...
export class PlanComposer {
  ...
  private async topologicalSort(
    nodes: Node[],
    hardEdges: Edge[]
  ): Promise<{ layers: string[][]; hasCycle: boolean }> {
    const nodeIds = new Set(nodes.map((n) => n.id));
    const inDegree = new Map<string, number>();
    const adjList = new Map<string, string[]>();
    const softScores = new Map<string, number>();
   ...
   */
  private filterNodes(nodes: Node[], request: PlanRequest): Node[] {
    ...
    const filtered = nodes.filter((node) => {
      ...
      if (request.requires) {
        const hasRequiredTag = request.requires.some((tag) =>
          node.requires.includes(tag)
        );
        if (hasRequiredTag) return true;
      }

      // Include if node produces any of the specified tags
      if (request.produces) {
        const hasProducedTag = request.produces.some((tag) =>
          node.produces.includes(tag)
        );
        if (hasProducedTag) return true;
      }
   ...
   */
  async composePlan(request: PlanRequest): Promise<PlanResult> {
    const graph = await readGraph(".");
    ...
    const { layers, hasCycle } = await this.topologicalSort(
      targetNodes,
      hardEdges
    );
    ...
    if (hasCycle) {
      throw new Error(
        "Cycle detected in hard dependency edges - cannot create safe plan"
      );
    }
    ...
    // Create reason text
    const reason =
      `Topological sort of ${targetNodes.length} nodes across ${layers.length} layers. ` +
    ...
    console.log(
      `Plan created with ${orderedNodeIds.length} nodes in ${layers.length} layers`
    );
...
let defaultPlanComposer: PlanComposer | null = null;

export function getPlanComposer(): PlanComposer {

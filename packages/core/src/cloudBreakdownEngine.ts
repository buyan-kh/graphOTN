...
import { getCloudStore } from "./cloudStore.js";
...
export class CloudBreakdownEngine {
  ...
  async breakdown(
    request: CloudBreakdownRequest
  ): Promise<CloudBreakdownResult> {
  ...
  private async createLLMBreakdown(
    prompt: string,
    maxNodes: number
  ): Promise<any> {
...
OUTPUT FORMAT (JSON):
{
  ...
  private convertToNodes(breakdown: any): Node[] {
  ...
  private createEdgesFromNodes(nodes: Node[]): Edge[] {
    const edges: Edge[] = [];
    const timestamp = new Date().toISOString();
    ...
    nodes.forEach((consumer) => {
      consumer.requires.forEach((requirement) => {
        nodes.forEach((producer) => {
          if (
            producer.produces.includes(requirement) &&
...
// Global instance
let cloudBreakdownEngine: CloudBreakdownEngine | null = null;

export function getCloudBreakdownEngine(): CloudBreakdownEngine {

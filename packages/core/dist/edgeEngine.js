 ...
 */
import { readGraph, addEdge } from "./fsStore.js";
export class EdgeEngine {
     ...
     */
    async inferSoftEdges(nodes, k = 5, threshold = 0.78) {
        ...
        for (const node of nodes) {
            ...
            try {
                ...
                const searchResults = await this.vectorStore.search(nodeEmbedding, k + 1); // +1 to account for self-match
                for (const result of searchResults) {
                    ...
                    candidates.push({
                        ...
                        nodeB: similarNode,
                        score: result.score,
                        evidence: `Semantic similarity: ${result.score.toFixed(4)} (>${threshold})`,
                    });
                }
            }
            catch (error) {
                console.warn(`Failed to find similar nodes for ${node.id}:`, error.message);
            }
        }
        ...
        for (const candidateAB of candidates) {
            ...
            const candidateBA = candidates.find((c) => c.nodeA.id === candidateAB.nodeB.id &&
                c.nodeB.id === candidateAB.nodeA.id);
            if (candidateBA) {
                // Create a consistent pair key (smaller ID first)
                const pairKey = candidateAB.nodeA.id < candidateAB.nodeB.id
                ...
                if (!mutualPairs.has(pairKey)) {
                    ...
                    const edgeAB = {
                        ...
                        type: "soft_semantic",
                        score: score,
                        evidence: `Mutual semantic similarity: ${score.toFixed(4)} (mutual nearest neighbors)`,
                        provenance: {
                            created_by: "edge_engine",
    ...
    async inferEdges(nodeIds) {
        ...
        for (const edge of [...hardEdges, ...softEdges]) {
            try {
                ...
                console.warn(`Failed to add edge ${edge.src} -> ${edge.dst}:`, error.message);
            }
        }
        console.log(`Created ${hardEdges.length} hard edges and ${softEdges.length} soft edges`);
        return {
            hardEdges,
            softEdges,
 ...
 */
let defaultEdgeEngine = null;
export function getEdgeEngine() {
